...
export class BookingService {
  ...
  async getAllActiveBooking(
    ...
    const [data, total] = await this.bookingRepository.findAndCount({
      where: [
        ...
        {
          bookingId: ILike(`%${searchTerm}%`),
          status: BookingStatus.COMPLETED,
        },
        {
          table: { tableId: ILike(`%${searchTerm}%`) },
          status: BookingStatus.COMPLETED,
        },
      ],
      relations: ['table'],
  ...
  async setAvialbleExpiredBooking() {
    ...
    const expiredBooking = await this.bookingRepository.find({
      where: { endDateTime: LessThan(today), status: BookingStatus.COMPLETED },
    });
    ...
    for (const booking of expiredBooking) {
      ...
      console.log(
        `✅ Booking ${booking.id} expired → Table ${booking.table.id} released`,
      );
    }
  }
  ...
  async deleteBooking(id: string) {
    ...
    if(!existingBooking){
      throw new NotFoundException("This booking id not found")
    }
